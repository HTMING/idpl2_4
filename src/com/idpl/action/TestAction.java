...
//		int repeat_number=Integer.parseInt(repeat);
		...
		int dataNumber_number=Integer.parseInt(dataNumber.substring(0,dataNumber.length()-2));
		if(number_number<parallel_number) 
		{
			note="The number of streams must be less than or equal to the number of files!";
			return result;
		}
		if(dataNumber_number>200)
		{
			note="data set is too big! data size < 200G!";
			return result;
		}
		if(dataSource.equals(dataDestination))
		{
			note="Source cannot be the same as Destination!";
			return result;
		}
